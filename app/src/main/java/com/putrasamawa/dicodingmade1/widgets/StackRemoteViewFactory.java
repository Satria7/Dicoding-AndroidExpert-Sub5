...
import com.putrasamawa.dicodingmade1.utils.Utils;

import java.util.concurrent.ExecutionException;

import static com.putrasamawa.dicodingmade1.db.MovieContract.CONTENT_URI;
import static com.putrasamawa.dicodingmade1.utils.Utils.EXTRA_ITEM;
...
public class StackRemoteViewFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    public RemoteViews getViewAt(int position) {
        ...
        if (cursor.moveToPosition(position)) {
            movieResult = new ItemMovie(cursor);
            Bitmap bmp;
            try {
                bmp = Glide.with(mContext)
                        .asBitmap()
                        .load(Utils.BASE_BACKDROP_URL_WIDGET + movieResult.getPoster_path())
                        .into(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL)
                        .get();
                rv.setImageViewBitmap(R.id.img_widget, bmp);
                rv.setTextViewText(R.id.tv_movie_title, movieResult.getOriginal_title());
